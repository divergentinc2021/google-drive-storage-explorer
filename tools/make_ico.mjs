/**
 * Build desktop/build/icon.ico from the PNGs in the same folder.
 *
 * electron-builder needs a real .ico for the NSIS target, and neither Windows nor
 * macOS ships ImageMagick, so the container is written by hand. A Vista-era ICO
 * may embed PNG data directly, which reduces this to a header plus a directory
 * plus the file bytes — no BMP encoding required.
 *
 * The PNGs come from assets/icon-512.png; see desktop/README.md.
 *
 * Usage: node tools/make_ico.mjs [sizes...]
 */
import { readFileSync, writeFileSync, existsSync } from 'node:fs';
import { dirname, join } from 'node:path';
import { fileURLToPath } from 'node:url';

const BUILD = join(dirname(fileURLToPath(import.meta.url)), '..', 'desktop', 'build');
...
console.log(`desktop/build/icon.ico written — ${images.length} sizes: ${sizes.join(', ')}`);
